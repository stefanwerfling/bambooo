 ...
 * ButtonMenu
 */
class ButtonMenu extends Button_js_1.Button {
    /**
     * div group
     * @protected
     */
     ...
     */
    constructor(element, icon = null, positonRight = false, type = Button_js_1.ButtonType.default, position = ButtonMenuPosition.relative, bnClass = ButtonClass_js_1.ButtonClass.default, shape = Button_js_1.ButtonShape.none) {
        ...
        if (icon !== null) {
            new Icon_js_1.Icon(this._element, icon);
        }
    ...
    addMenuItem(title, clickFn, icon = null) {
        ...
        if (icon !== null) {
            new Icon_js_1.Icon(item, icon);
            item.append('&nbsp;&nbsp;');
        }
        const textSpan = jQuery('<span></span>').appendTo(item);
        LangText_js_1.LangText.addLangText(textSpan, title);
        item.on('click', (event) => {
            clickFn();
